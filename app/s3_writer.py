...
class WindowedS3Writer:
    def __init__(
        ...
        *,
        ...
        if window_minutes not in (30, 15, 60):
            logger.warning("nonstandard window_minutes=%s; proceeding", window_minutes)
        self.bucket = bucket
    ...
    def _start_new_window(self, when: datetime) -> None:
        ...
        self._upload_offset = 0
        self._parts.clear()
        logger.debug("s3-window-start key=%s", key)
        if self.use_marker:
            marker = f"{prefix}/uploading.marker"
            try:
                self.s3.put_object(Bucket=self.bucket, Key=marker, Body=b"")
            except Exception:
                logger.exception("s3-marker-put-failed marker=%s", marker)
    ...
    def _upload_available_parts(self) -> None:
        while (self._buffer.tell() - self._upload_offset) >= self.part_size_bytes:
            ...
            logger.debug("s3-upload-part key=%s part=%s bytes=%s", self._current_key(), part_num, len(data))
    ...
    def _finalize_current(self) -> None:
        ...
        try:
            ...
            if remaining > 0:
                ...
                part_num = len(self._parts) + 1
                logger.debug("s3-upload-last-part key=%s part=%s bytes=%s", self._current_key(), part_num, len(data))
                resp = self.s3.upload_part(
                    Bucket=self.bucket,
                    Key=self._current_key(),
                    PartNumber=part_num,
            ...
            logger.debug("s3-complete key=%s parts=%s", self._current_key(), len(self._parts))
            ...
            logger.exception("s3-complete-failed key=%s", self._object_key)
            try:
                if self._upload_id:
                    self.s3.abort_multipart_upload(Bucket=self.bucket, Key=self._current_key(), UploadId=str(self._upload_id))
            except Exception:
                logger.exception("s3-abort-failed key=%s", self._object_key)
        finally:
            if self.use_marker and self._current_window_start and self._current_window_end:
                ...
                try:
                    ...
                    logger.exception("s3-marker-delete-failed marker=%s", marker)
            self._buffer = io.BytesIO()
            self._upload_offset = 0
